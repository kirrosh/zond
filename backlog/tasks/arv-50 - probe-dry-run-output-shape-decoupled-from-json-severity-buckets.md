...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 probe security --dry-run --json: data.endpoints[] = [{path, method, planned: boolean, classes_planned: string[], fields_planned: string[], skip_reason: null | 'no-body' | 'no-matched-field' | 'isolated-protected'}]
- [x] #2 data.severity отсутствует (или явно null с комментарием 'severity not applicable in dry-run')
- [x] #3 data.summary содержит { totalEndpoints, planned, skipped } — отдельные счётчики
- [x] #4 Resend F1-15 fixture-test: dry-run возвращает 14 endpoints planned:true, 18 planned:false с skip_reason
- [x] #5 Schema published в docs/json-schema/probe-dry-run.schema.json
<!-- AC:END -->
