...
id: TASK-271
title: >-
  bootstrap --apply --seed: непрозрачный вывод — что пробовали, что не
  получилось, сколько пасов
status: Done
assignee: []
...
- [ ] #1 Bootstrap печатает таблицу/список FK с per-key статусом (`already`, `discovered`, `seeded`, `failed:reason`).
- [ ] #2 Summary line содержит N пасов и причину остановки.
- [ ] #3 No-op случай (`Filled X/X`) явно помечен как `nothing to do` + не вводит в заблуждение «1 pass, 0 new».
- [ ] #4 `--dry-run` vs `--apply` визуально различимы (prefix `[plan]` vs `[exec]` или явный header).
