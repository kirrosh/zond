...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 interface Probe { name; description; commonFlags; dryRun(ctx): EndpointPlan[]; run(ctx): ProbeResult; report(format: 'markdown'|'json', results): string|object } в src/core/probe/types.ts
- [ ] #2 core/probe/registry.ts валидирует все registered classes на старте — отсутствующий слот → throw at boot с понятным сообщением 'Probe X is missing required method dryRun'
- [ ] #3 static, mass-assignment, security — все три рефакторнуты под BaseProbe abstract class
- [ ] #4 tests/contracts/probe-interface.test.ts: mock Probe class без dryRun → registry throws; mock с полным contract → boots successfully
- [ ] #5 Существующие e2e тесты probe-команд остаются green (zero behavior change на этом этапе)
<!-- AC:END -->
