...
id: TASK-280
title: >-
  coverage --json: отдельные covered2xx / coveredButNon2xx / unhit (не только
  coveredEndpoints + partialEndpoints)
status: Done
assignee: []
created_date: '2026-05-08 19:00'
updated_date: '2026-05-08 13:54'
labels:
  - feedback-loop
  - api-sentry
  - coverage
...
- [ ] #1 JSON envelope `coverage` содержит `covered2xxEndpoints`, `coveredButNon2xxEndpoints`, `unhitEndpoints` как отдельные массивы.
- [ ] #2 `totals` включает `covered2xx`/`coveredButNon2xx`/`unhit` параллельно с существующими полями.
- [ ] #3 Семантика 1-в-1 совпадает с не-JSON выводом (`✅ N covered (passing 2xx)` == `totals.covered2xx == covered2xxEndpoints.length`).
- [ ] #4 Обратная совместимость: `coveredEndpoints`/`partialEndpoints` либо deprecated с warning, либо drop'нуты под mention в CHANGELOG (решение в рамках envelope-policy module).
- [ ] #5 Регрессионный test на JSON snapshot для fixture-run с 3 endpoints (один 2xx, один 4xx, один не hit).
- [ ] #6 ZOND.md / `--help` для `coverage`: документирует категории.
