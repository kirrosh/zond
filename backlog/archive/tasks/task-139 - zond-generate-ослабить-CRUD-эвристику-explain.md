...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 CRUD-detector принимает `POST /<r>/` и распознаёт id-подобные поля
      (`id|uuid|slug|version|key|name`).
- [ ] #2 `zond generate --explain` показывает per-resource verdict и причину.
- [ ] #3 На fixture-спеке с Sentry-подобной формой (alert-rules, dashboards)
      генерируется CRUD-suite, где раньше генерировалось 0.
- [ ] #4 Не регрессирует на классических OpenAPI-спеках (тест на
      `petstore.json`).
- [ ] #5 CHANGELOG.
<!-- AC:END -->
...
<!-- SECTION:NOTES:BEGIN -->
1) detectCrudGroups: stripTrailingSlash + opt-trailing-slash regex — Sentry-style POST /x/ + GET /x/{id} теперь матчится. 2) getCaptureField расширен: path-param name match (slug, rule_id↔id), затем id, затем string-поля slug/uuid/key/version/name, затем integer/uuid by type. 3) Новая функция detectCrudGroupsWithDiagnostics + zond generate --explain печатает таблицу resource|post|get/{id}|put/patch|delete|list|verdict|reason без записи файлов. 6 новых юнит-тестов. CHANGELOG, ZOND.md, skill (Phase 2) обновлены.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
TASK-139: CRUD-detector принимает trailing-slash и id-подобные поля, добавлен --explain для диагностики. 936/936 тестов зелёных.
