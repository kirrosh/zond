...
> Цель: collapse всё в single-page dashboard. Убрать management screens — CLI/MCP territory.

### Результат

Single-page dashboard: collection selector → env selector → Run Tests → inline results + coverage + history.

| # | Задача | Описание | Статус |
|---|--------|----------|--------|
| 1 | **Single-page dashboard** | API selector, env selector, Run Tests button, HTMX panel endpoints | ✅ |
| 2 | **Results panel** | Suite-grouped steps, auto-expand failures, capture badges, chain visualization | ✅ |
| 3 | **Coverage panel** | OpenAPI spec coverage %, all endpoints list (covered ✓ / uncovered ✗) | ✅ |
| 4 | **Run history** | Compact list, click to load results, Load more pagination | ✅ |
| 5 | **Export** | JUnit XML + JSON export buttons on every run | ✅ |
| 6 | **Remove 8 routes** | Deleted: collections, suites, ai-generate, environments, explorer, trend-chart | ✅ |
| 7 | **Shared views** | Extracted `views/results.ts` — reusable rendering for dashboard + /runs/:id | ✅ |
| 8 | **Auto-select defaults** | Single collection auto-selected; scoped env auto-selected in dropdown | ✅ |
...
| M26 (WebUI Simplification) | Single-page dashboard, removed 8 routes, shared views, coverage panel | ✅ Done |
